...
echo '<section class="slideshow-d slideshow-d_banners">';
...
'<div class="gall-d swiper-container" data-gall-autoplay="8000" data-gall-simulate-touch="true">
	<div class="gall-d__list swiper-wrapper">';
		...
		{
			echo
			'<a class="slide-d slide-d_cover swiper-slide'.(! empty($row['link']) ? ' js_bs_counter bs_counter" href="'.$row['link'].'" rel="'.$row['id'].'"'.(! empty($row['target_blank']) ? ' target="_blank"' : '') : '').'"'.(! empty($row['image']) ? ' style="background-image: url('.BASE_PATH.USERFILES.'/bs/'.$row['image'].');' : '').'">
				<div class="slide-d__content">';
					...
					if(! empty($row['text']))
					{
						echo '<object class="_text">'.$row['text'].'</object>';
					}

					//вывод ссылки на баннер, если задана
					if (! empty($row['link']))
					{
						// echo '<a href="'.$row['link'].'" class="button-d button-d_narrow js_bs_counter bs_counter" rel="'.$row['id'].'"'.(! empty($row['target_blank']) ? ' target="_blank"' : '').'>'.$this->diafan->_('Заказать').'</a>';
					}
