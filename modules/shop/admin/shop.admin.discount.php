...
class Shop_admin_discount extends Frame_admin
{
	...
	*/
	...
	public $variables = array (
		'main' => array (
			...
			'periodicity' => array(
				'type' => 'function',
				'name' => 'Периодичность',
				'help' => 'Если выбрать дни недели, то скидка будет применяться только в указанные дни. Если ограничить время действия скидки, то скидка будет применяться только в указанное время.',
			),
			'act' => array(
				'type' => 'checkbox',
	...
	/**
	 * Редактирование поля "Периодичность"
	 * @return void
	 */
	public function edit_variable_periodicity()
	{
		if($this->diafan->id)
		{
			$rows = DB::query_fetch_all("SELECT * FROM {shop_discount_periodicity} WHERE discount_id=%d", $this->diafan->id);
			foreach($rows as $row)
			{
				if($row["type"] == 'week' && $row["day_week"])
				{
					$periodicity["week"][$row["day_week"]] = true;
				}
				if($row["type"] == 'time' && $row["time_start"])
				{
					$periodicity["time"] = $row;
				}
			}
		}
		echo '
		<div class="unit" id="periodicity">
		<div class="infofield">'.$this->diafan->variable_name().$this->diafan->help().'</div>
		<p>'.$this->diafan->_('Дни недели').': ';
		$select = array(
			2 => 'пн',
			3 => 'вт',
			4 => 'ср',
		...
		{
			echo ' <input type="checkbox" name="periodicity_week[]" id="periodicity_week_'.$k.'" value="'.$k.'"'.(empty($periodicity["week"]) || ! empty($periodicity["week"][$k]) ? ' checked' : '').' class="js_periodicity_week"> <label for="periodicity_week_'.$k.'">'.$this->diafan->_($v).'</label>';
		}
		echo '</p><p>'.$this->diafan->_('Время');
		echo ': <input type="text" name="periodicity_time_start" value="';
		if(! empty($periodicity["time"]["time_start"]))
		{
			echo date("H:i", mktime(0,0,0) + $periodicity["time"]["time_start"]);
		}
		echo '" class="timecalendar" onlytime="true"> - <input type="text" name="periodicity_time_finish" value="';
		if(! empty($periodicity["time"]["time_finish"]))
		{
			echo date("H:i", mktime(0,0,0) + $periodicity["time"]["time_finish"]);
		}
		echo '" class="timecalendar" onlytime="true">';
		echo '</p></div>';
	}
	...
	/**
	 * Сохранение поля "Периодичность"
	 * @return void
	 */
	public function save_variable_periodicity()
	{
		if(! $this->diafan->is_new)
		{
			$rows = DB::query_fetch_all("SELECT * FROM {shop_discount_periodicity} WHERE discount_id=%d", $this->diafan->id);
		...
		if(empty($_POST["periodicity_week"]) || ! is_array($_POST["periodicity_week"]) || count($_POST["periodicity_week"]) > 6)
		{
			if(! empty($periodicity["week"]))
			{
				DB::query("DELETE FROM {shop_discount_periodicity} WHERE discount_id=%d AND type='week'", $this->diafan->id);
			}
			$this->set_periodicity('week', false);
		}
		else
		{
			DB::query("DELETE FROM {shop_discount_periodicity} WHERE discount_id=%d AND type='week' AND day_week='0'", $this->diafan->id);
			foreach($_POST["periodicity_week"] as $w)
			{
				if(empty($periodicity["week"][$w]))
				{
					DB::query("INSERT INTO {shop_discount_periodicity} (discount_id, type, day_week) VALUES (%d, 'week', '%d')", $this->diafan->id, $w);
				}
			}
			foreach($periodicity["week"] as $w => $dummy)
			{
				if(! in_array($w, $_POST["periodicity_week"]))
				{
					DB::query("DELETE FROM {shop_discount_periodicity} WHERE discount_id=%d AND type='week' AND day_week='%d'", $this->diafan->id, $w);
				}
			}
			$this->set_periodicity('week', true);
		}

		if(empty($_POST["periodicity_time_start"]) || empty($_POST["periodicity_time_finish"]))
		{
			if(! empty($periodicity["time"]["time_start"]))
			{
				DB::query("UPDATE {shop_discount_periodicity} SET time_start=0, time_finish=0 WHERE id=%d", $periodicity["time"]["id"]);
			}
			$this->set_periodicity('time', false);
		}
		else
		{
			$time_start = 0;
			if($_POST["periodicity_time_start"] == "00:00")
			{
				$time_start = 1;
			}
			else
			{
				$t = explode(":", trim($_POST["periodicity_time_start"]));
				if(count($t) == 2)
				{
					$time_start = mktime($t[0], $t[1], 0) - mktime(0,0,0);
				}
			}
			$time_finish = 0;
			if($_POST["periodicity_time_finish"] == "00:00")
			{
				$time_finish = 1;
			}
			else
			{
				$t = explode(":", trim($_POST["periodicity_time_finish"]));
				if(count($t) == 2)
				{
					$time_finish = mktime($t[0], $t[1], 0) - mktime(0,0,0);
				}
			}
			if($time_start > $time_finish)
			{
				$time_finish2 = $time_start;
				$time_start = $time_finish;
				$time_finish = $time_finish2;
			}
			if(! empty($periodicity["time"]))
			{
				DB::query("UPDATE {shop_discount_periodicity} SET time_start=%d, time_finish=%d WHERE id=%d", $time_start, $time_finish, $periodicity["time"]["id"]);
			}
			else
			{
				DB::query("INSERT INTO {shop_discount_periodicity} (discount_id, type, time_start, time_finish) VALUES (%d, 'time', %d, %d)", $this->diafan->id, $time_start, $time_finish);
				}
			$this->set_periodicity('time', true);
		}
	}

	/**
	 * Сохранение настройки "Периодичность"
	 * @return void
	 */
	private function set_periodicity($type, $value)
	{
		if($type == 'week')
		{
			if($value)
			{
				$rows = DB::query_fetch_all("SELECT * FROM {shop_discount} WHERE id NOT IN (SELECT discount_id FROM {shop_discount_periodicity} WHERE type='week')");
				foreach($rows as $i => $row)
				{
					DB::query("INSERT INTO {shop_discount_periodicity} (discount_id, type) VALUES (%d, 'week')", $row["id"]);
				}
				if(! $this->diafan->configmodules('periodicity_discount_week', 'shop'))
				{
					$this->diafan->configmodules('periodicity_discount_week', 'shop', 0, false, '1');
				}
			}
			else
			{
				if(! DB::query_result("SELECT id FROM {shop_discount_periodicity} WHERE type='week' AND day_week<>'0' LIMIT 1"))
				{
					if($this->diafan->configmodules('periodicity_discount_week', 'shop'))
					{
						$this->diafan->configmodules('periodicity_discount_week', 'shop', 0, false, '');
					}
				}
			}
		}
		if($type == 'time')
		{
			if($value)
			{
				if(! $this->diafan->configmodules('periodicity_discount_time', 'shop'))
				{
					$rows = DB::query_fetch_all("SELECT * FROM {shop_discount} WHERE id NOT IN (SELECT discount_id FROM {shop_discount_periodicity} WHERE type='time')");
					foreach($rows as $i => $row)
					{
						DB::query("INSERT INTO {shop_discount_periodicity} (discount_id, type) VALUES (%d, 'time')", $row["id"]);
					}
					$this->diafan->configmodules('periodicity_discount_time', 'shop', 0, false, '1');
				}
			}
			else
			{
				if(! DB::query_result("SELECT id FROM {shop_discount_periodicity} WHERE type='time' AND time_start<>'0' LIMIT 1"))
				{
					if($this->diafan->configmodules('periodicity_discount_time', 'shop'))
					{
						$this->diafan->configmodules('periodicity_discount_time', 'shop', 0, false, '');
					}
				}
			}
		}
	...
	public function delete($del_ids)
	{
		...
		$this->diafan->del_or_trash_where("shop_discount_periodicity", "discount_id IN (".implode(",", $del_ids).")");
	}
}
