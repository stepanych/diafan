...
// начало уровня меню
echo '<ul class="nav-d__menu menu-d">';
foreach ($result["rows"][$result["parent_id"]] as $row)
{
	if ($row["active"])
	{
		// начало пункта меню для текущей страницы
		echo '<li class="item-d item-d_main item-d_current item-d_active">';
	}
	elseif ($row["active_child"])
	{
		// начало пункта меню для активного дочернего пункта
		echo ' <li class="item-d item-d_main'.($result["show_all_level"] && ! empty($result["rows"][$row["id"]]) ? ' item-d_parent item-d_active' : '').'">';
	}
	elseif ($row["children"])
	{
		// начало пункта меню для элемента -родителя
		echo ' <li class="item-d item-d_main'.($result["show_all_level"] && ! empty($result["rows"][$row["id"]]) ? ' item-d_parent' : '').'">';
	}
	else
	{
		// начало любого другого пункта меню
		echo '<li class="item-d item-d_main">';
	}
	...
	)
	{
		if ($row["othurl"])
		{
			echo '<a class="item-d__link item-d__link_main link-d'.(! empty($row["active"]) || ! empty($row["active_child"]) ? ' active' : '').'" href="'.$row["othurl"].'"'.$row["attributes"].'>';
		}
		else
		{
			echo '<a class="item-d__link item-d__link_main link-d'.(! empty($row["active"]) || ! empty($row["active_child"]) ? ' active' : '').'" href="'.BASE_PATH_HREF.$row["link"].'"'.$row["attributes"].'>';
		}
	}
	else
	{
		echo '<a class="item-d__link item-d__link_main link-d"'.$row["attributes"].'>';
	}
	...
	if ($result["show_all_level"] && ! empty($result["rows"][$row["id"]]))
	{
		echo
