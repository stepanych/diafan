...
class Order_inc extends Diafan
{
		...
		foreach ($rows as $row)
		{
			...
			if(! empty($additional_costs[$row["id"]]))
			{
				foreach($additional_costs[$row["id"]] as $a)
				{
					...
					{
						$row_price["old_price"] += $a["summ"] / $row["count"]; 
					}
	...
	{
		$this->diafan->_delivery->set_status($order["id"], $status["id"]);
		...
		if(! empty($status["custom_mail"]))
		{
			...
			if($status["subject_mail".$lang_id] && $status["text_mail".$lang_id])
			{
				if(! $status["send_mail"])
				{
					$user_mail = DB::query_result(
				...
				if($user_mail)
				{
					$subject = str_replace(array('%title', '%url'), array(TITLE, BASE_URL), $status["subject_mail".$lang_id]);

					$message = str_replace(array('%title', '%url', '%order', '%status'), array (TITLE, BASE_URL, $order["id"], $status["name".$lang_id]), $status["text_mail".$lang_id]);
